...
/**
 * Backfill hyperlight sub-books for hyperlights created before the sub-book system.
 *
 * PURPOSE:
 * Hyperlights created before the sub-book system only have plain text in the
 * hyperlights.annotation column — no library record, no nodes, no preview_nodes.
 * When a non-owner opens one of these, subBookLoader.js synthesises a local node
 * and fires createSubBookOnBackend, which returns 403.
 *
 * This command materialises every unmigrated hyperlight as a proper sub-book
 * (library record + node) and writes preview_nodes back to the hyperlight row.
 ...
 *
 * USAGE:
 * php artisan hyperlights:backfill-sub-books              # Run migration
 * php artisan hyperlights:backfill-sub-books --dry-run    # Preview without changes
 *
 * IDEMPOTENT: Safe to re-run. Hyperlights that already have preview_nodes are
 * skipped. Hyperlights with nodes but no preview_nodes get preview_nodes refreshed.
 */
class BackfillHyperlightSubBooks extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'hyperlights:backfill-sub-books
     ...
     * @var string
     */
    protected $description = 'Backfill sub-book library records and nodes for pre-existing hyperlights';
    ...
    public function handle(): int
    {
        ...
        $total = $hyperlights->count();

        if ($total === 0) {
            $this->info('No hyperlights need backfilling.');
            return 0;
        }

        $this->info("Found {$total} hyperlight(s) to process.");

        if ($dryRun) {
            $this->newLine();
            $this->table(
                ['book', 'hyperlight_id', 'annotation (truncated)'],
        ...
        foreach ($hyperlights as $hyperlight) {
            try {
                $subBookId = $hyperlight->book . '/' . $hyperlight->hyperlight_id;

                // Creator comes from the hyperlight record itself (NOT parent library).
                $creator      = $hyperlight->creator      ?? null;
                $creatorToken = $hyperlight->creator_token ?? null;

                if (!$creator && !$creatorToken) {
                    $this->newLine();
                    $this->warn("Skipping {$subBookId}: no creator info on hyperlight");
                    $skipped++;
                    $bar->advance();
                    continue;
                }
                ...
                if ($existingNode) {
                    $this->refreshPreviewNodes($subBookId, $hyperlight->book, $hyperlight->hyperlight_id);
                    $refreshed++;
                    $bar->advance();
                    continue;
                }

                // Upsert library record (visibility is always 'public' for hyperlights).
                $this->admin->table('library')->updateOrInsert(
                    ...
                    [
                        ...
                        'visibility'    => 'public',
                        'listed'        => false,
                        'title'         => "Annotation: {$hyperlight->hyperlight_id}",
                        'type'          => 'sub_book',
                        'has_nodes'     => true,
                ...
                // Write preview_nodes to the hyperlight row using the new node.
                $previewNodes = [[
                    'book'        => $subBookId,
                    'chunk_id'    => 0,
                    'startLine'   => 1.0,
                ...
                $this->admin->table('hyperlights')
                    ->where('book', $hyperlight->book)
                    ->where('hyperlight_id', $hyperlight->hyperlight_id)
                ...
                $errors++;
                $this->newLine();
                $this->error("Error processing {$hyperlight->book}/{$hyperlight->hyperlight_id}: {$e->getMessage()}");
            }
     ...
     * Reads the first nodes and writes an enriched JSON array back to the hyperlight row.
     */
    private function refreshPreviewNodes(string $subBookId, string $parentBook, string $hyperlightId): void
    {
        $nodeRows = $this->admin
            ->table('nodes')
