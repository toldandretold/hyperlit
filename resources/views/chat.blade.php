...
{{-- Experimental homepage: lava-lamp animated background + centered glass hero.
     pageType is 'home' so every home-registered component initializes as usual.
     Deliberate differences from home.blade.php:
       - no $prerenderHtml / $jsonLd (this page is experimental, not indexed)
       - no .arranger-button has the `active` class and there are no <main> ids,
         which is exactly what suppresses homepageDisplayUnit's initial auto-load;
         content loads only when a button is pressed (it creates the .main-content)
       - #app-container carries .chat-page (styles + chatHero behavior key off it)
       - #lava-lamp-mount enables the lavaLamp component (inert elsewhere) --}}

@section('styles')
    @vite(['resources/css/app.css', 'resources/css/reader.css', 'resources/css/highlight-div.css', 'resources/css/containers.css', 'resources/css/buttons.css', 'resources/css/form.css', 'resources/css/alert.css', 'resources/css/layout.css', 'resources/css/chat.css'])
@endsection

@section('content')

<div id="app-container" class="chat-page">
