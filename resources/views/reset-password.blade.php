    ...
    <form id="reset-password-form" autocomplete="on" style="display: block;">
      ...
      <div style="margin-bottom: 14px;">
        <input type="email" id="reset-email" name="email" placeholder="Email" required autocomplete="email"
               value="{{ $email }}"
               style="width: 100%; padding: 9px; border-radius: 4px; border: none; background: var(--container-bg, #111); color: var(--color-text); box-sizing: border-box; font-size: 14px;">
      ...
      <button type="submit" id="reset-submit"
              style="width: 100%; padding: 10px; background: var(--color-accent); color: var(--color-background); border: none; border-radius: 4px; cursor: pointer; font-size: 14px; margin-bottom: 10px;">
        Set New Password
      </button>
    </form>
...
document.getElementById('reset-password-form').addEventListener('submit', async function (e) {
  ...
  try {
    ...
    if (response.ok && data.success) {
      // Show success and redirect to home after a moment
