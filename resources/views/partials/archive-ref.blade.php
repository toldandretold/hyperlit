{{-- Top-right archive button (reader cloudRef chrome parity, distinct id):
     opens the archive panel — the citation/source container for this WHOLE
     library/journal corpus with the vault/SQLite downloads. Expects
     $archiveScopeType (user|journal|archive) and $archiveScopeId. NOT
     #cloudRef: the source-container singleton binds that id at import time
     and scopes it to the per-book `book` global, which is garbage on hero
     pages. DOM position right after the top-left cluster = Tab order
     top-left → top-right (WCAG 2.4.3; position:fixed makes placement free). --}}
<div id="topRightContainer" class="loading">
    <button type="button" id="archiveRef" aria-label="Archive citation and downloads"
            data-scope-type="{{ $archiveScopeType }}" data-scope-id="{{ $archiveScopeId }}">
        <svg
          id="archiveRef-svg"
