...
def clean_html(output_file):
    with open(output_file, 'r', encoding='utf-8') as f:
        ...
        # Step 5: Remove href attributes from <a> tags inside <h1>, <h2>, <h3> tags
        for header in soup.find_all(['h1', 'h2', 'h3']):
            for link in header.find_all('a', href=True):
                if 'id' in link.attrs:
                    # Remove only the href attribute, keep the id intact
                    del link['href']
                else:
                    # If no id, remove the entire <a> tag but keep its text
                    link.unwrap()

        # Step 6: Remove all class attributes from all remaining tags
...
def remove_old_toc(output_file):
    ...
    if toc_start:
        ...
        siblings_to_remove = []
        found_end = None  # To store the element we stop at (next heading or <br/>)
