...
function buildBarChartSvg(data: any, colors: any) {
  ...
  data.forEach((item: any, i: any) => {
    ...
    const label = (SHORT_LABELS as any)[item.label] || item.label;
...
function buildDonutChartSvg(data: any, colors: any) {
  const svgNS = 'http://www.w3.org/2000/svg';
  const width = 300;
  ...
  const notFound = data.find((d: any) => /not found/i.test(d.label))?.count ?? 0;
...
export function renderCharts(container: any) {
  // Verdict bar charts
  ...
  coverageTables.forEach((table: any) => {
    const rows = table.querySelectorAll('tbody tr');
    const data = Array.from(rows).map((row: any) => {
      const cells = row.querySelectorAll('td');
      return {
