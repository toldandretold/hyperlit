 ...
 */
export function parseMarkdownIntoChunksInitial(markdown: any) {
  ...
  (footnoteData as any).pairs.forEach((pair: any) => {
  ...
  elements.forEach((el: any, index: number) => {
    ...
    nestedElements.forEach((nested: any) => {
    ...
    const nodeFootnotes = (footnoteData as any).pairs.filter((pair: any) => {
    ...
    if (nodeFootnotes.length > 0) {
      node.footnotes = nodeFootnotes.map((pair: any) => ({
 ...
 */
export function convertMarkdownToHtml(markdown: any) {
  const lines = markdown.split("\n");
  let htmlOutput = "";
  let inMathBlock = false;
  let mathBlockLines: any[] = [];
 ...
 */
function encodeMath(latex: any) {
  return btoa(unescape(encodeURIComponent(latex)));
}

export function parseInlineMarkdown(text: any) {
  if (!text) return "";
 ...
 */
export function renderBlockToHtml(block: any) {
