 ...
 * @returns {boolean}
 */
export function listItemIsEmpty(li) {
  if (!li) return false;
 ...
 */
export function placeCaretAtEndOfListItem(li, selection = null) {
  if (!li) return;
 ...
 * @param {Selection} [selection] - optional selection (defaults to window.getSelection())
 */
export function placeCaretInEmptyListItem(li, selection = null) {
  if (!li) return;
  const sel = selection || window.getSelection();
