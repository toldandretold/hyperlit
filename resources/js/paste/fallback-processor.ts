 ...
 * @param {string} bookId - Book identifier
 */
export async function saveFootnotesToIndexedDB(footnotes: any, bookId: any) {
 ...
 * @param {string} bookId - Book identifier
 */
export async function saveReferencesToIndexedDB(references: any, bookId: any) {
 ...
 * @param {string} bookId - Book identifier
 */
async function syncFootnotesToPostgreSQL(footnotes: any, bookId: any) {
 ...
 * @param {string} bookId - Book identifier
 */
async function syncReferencesToPostgreSQL(references: any, bookId: any) {
  if (!references || references.length === 0) return;

  // Validate and filter references before syncing
  const validReferences: any[] = [];

  references.forEach((reference: any, index: any) => {
...
export { preprocessHTMLContent, isRealLink } from './utils/html-preprocessor';
