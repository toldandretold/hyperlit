 ...
 */
export function handleSmallPaste(event: any, htmlContent: any, plainText: any, nodeCount: any, book: any) {
  ...
  if (!finalHtmlToInsert && plainText) {
    ...
    if (parts.length === 1 && currentBlock) {
      ...
      finalHtmlToInsert = parts.map((p: any) => `<p>${p}</p>`).join("");
    }
  }
  ...
  let cursorBefore = { elementId: currentBlock.id, offset: 0 };
  {
    ...
    if (sel && sel.rangeCount > 0) {
      ...
      } catch (e: any) { /* cursor may not be inside block (will be moved for H1) */ }
    }
  }
 ...
 */
function _inlinePaste(currentBlock: any, html: any, book: any, undoManager: any, cursorBefore: any) {
 ...
 */
function _blockPaste(currentBlock: any, html: any, book: any, undoManager: any, cursorBefore: any) {
  const oldHTML = currentBlock.innerHTML;
  ...
  if (undoManager && editableSelector) {
    ...
    const added = insertedElements
      .filter((el: any) => el.id && /^\d+(\.\d+)*$/.test(el.id))
      .map((el: any) => ({
        id: el.id,
        html: el.innerHTML,
