...
import { normalizeListItems } from './normalizer';
 ...
 */
export function preprocessMarkdownFootnotes(text: any) {
  ...
  const definedIds = new Set(footnoteDefinitions.map((d: any) => d.identifier));
  ...
  processedText = processedText.replace(/\[\^(\d+)\]/g, (fullMatch: any, num: any) => {
 ...
 */
export function footnoteDefinitionsToHtml(definitions: any) {
 ...
 */
export async function processMarkdownInChunks(text: any, onProgress: any) {
