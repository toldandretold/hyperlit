 ...
 */
export function extractFootnoteIdsFromHtml(htmlContent: any) {
 ...
 * @returns {{id: string, marker: string}[]} - Array of unique footnote objects
 */
export function extractFootnoteIdsFromElement(element: any) {
  if (!element) return [];
  ...
  element.querySelectorAll('sup.footnote-ref[id]').forEach((sup: any) => {
  ...
  element.querySelectorAll('sup[fn-count-id] a.footnote-ref, a.footnote-ref').forEach((link: any) => {
