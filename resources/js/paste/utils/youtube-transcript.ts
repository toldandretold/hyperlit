 ...
 * Detects YouTube transcripts from paste and formats them for readability
 */
 ...
 *
 * @param {string} plainText - Plain text from clipboard
 * @param {string} rawHtml - HTML from clipboard
 * @returns {{isYouTube: boolean, source: string|null}} - Detection result
 */
export function detectYouTubeTranscript(plainText: any, rawHtml: any) {
 ...
 * @param {string} rawHtml - HTML transcript (if available)
 * @param {string} source - Detection source ('html' or 'plaintext')
 * @returns {string} - Formatted transcript as markdown-style text
 */
export function transformYouTubeTranscript(plainText: any, rawHtml: any, source: any) {
  let extractedText = '';
