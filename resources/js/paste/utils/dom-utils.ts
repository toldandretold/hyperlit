...
import { isStructuralBlockTag, STRUCTURAL_BLOCK_TAGS } from '../../utilities/blockElements';
 ...
 */
export function isBlockElement(tagName: any) {
 ...
 */
export function replaceTag(el: any, newTagName: any, doc = document) {
...
export function unwrapNonContentContainers(container: any, doc = document) {
  const selector = NON_CONTENT_BLOCK_TAGS.join(', ');
  const containers = Array.from<any>(container.querySelectorAll(selector)).reverse();
 ...
 * @param {Document} doc - Document context
 */
export function wrapLooseNodes(container: any, doc = document) {
  const nodesToProcess = Array.from<any>(container.childNodes);
 ...
 */
export function createTempDOM(html: any) {
 ...
 */
export function isRealLink(href: any) {
 ...
 */
export function removeEmptyBlocks(container: any) {
  container.querySelectorAll("p, blockquote, h1, h2, h3").forEach((el: any) => {
    if (
      !el.textContent.trim() &&
 ...
 */
export function stripAttributes(container: any, idPrefix = '') {
 ...
 * @param {Document} doc - Document context
 */
export function groupInlineElements(container: any, doc = document) {
  const looseInlineElements = Array.from<any>(container.childNodes).filter((node: any) =>
 ...
 */
export function visuallyStartsWith(htmlContent: any, textPattern: any) {
 ...
 */
export function isReferenceSectionHeading(headingText: any) {
