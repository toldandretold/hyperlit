...
import { isStructuralBlockTag } from '../../utilities/blockElements';
 ...
 */
export function parseHtmlToBlocks(htmlContent: any) {
  ...
  Array.from<any>(tempDiv.childNodes).slice(0, 10).forEach((node, i) => {
  ...
  const blocks: any[] = [];
  let inlineAccumulator: any[] = []; // Accumulate consecutive inline elements
  ...
  Array.from<any>(tempDiv.childNodes).forEach((node, nodeIndex) => {
    if (node.nodeType === Node.ELEMENT_NODE && isBlockElement(node.tagName)) {
      ...
      if (brSeparatedParts.length > 1 && !isUnsplittableBlock && !containsBlockElements) {
        ...
        brSeparatedParts.forEach((part: any) => {
          ...
          if (trimmedPart) {
            ...
            Array.from<any>(wrapper.childNodes).forEach((node: any) => {
              if (node.nodeType === Node.ELEMENT_NODE) {
