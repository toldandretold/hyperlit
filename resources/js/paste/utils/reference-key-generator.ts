 ...
 */
export function generateReferenceKeys(text: any, contextText = '', formatType = 'general') {
  ...
  const keys: any[] = [];
  const addKey = (key: any) => { if (key && !keys.includes(key)) keys.push(key); };
  ...
  if (authorSource) {
    ...
    const surnames = (sourceText.match(/\b[A-Z][a-zA-Z'-]+\b/g) || [])
      .filter((s: any) => !['And', 'The', 'For', 'In', 'An', 'On', 'As', 'Ed', 'Of', 'See', 'Also'].includes(s))
      .filter((s: any) => !acronyms.includes(s)) // Don't re-process acronyms as surnames
      .map((s: any) => s.toLowerCase().replace("'s", ""));
