...
export class WileyProcessor extends BaseFormatProcessor {
  ...
  async extractFootnotes(dom: any, bookId: any) {
    ...
    noteElements.forEach((element: any, index: any) => {
  ...
  async transformStructure(dom: any, bookId: any) {
    ...
    uiSelectors.forEach((selector: any) => {
      dom.querySelectorAll(selector).forEach((el: any) => el.remove());
    });

    // STEP 4: Clean up empty reference lists
    dom.querySelectorAll('ul.article__references, ol.article__references').forEach((list: any) => {
