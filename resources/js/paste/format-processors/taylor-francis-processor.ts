...
export class TaylorFrancisProcessor extends BaseFormatProcessor {
  ...
  async extractFootnotes(dom: any, bookId: any) {
    ...
    notesHeadings.forEach((heading: any) => {
              ...
              paragraphs.forEach((p: any) => {
                ...
                if (match) {
                  ...
                  if (upperRid.startsWith('EN')) {
                    (footnote as any).enId = footnoteRid;
                  } else if (upperRid.startsWith('FN')) {
                    (footnote as any).fnId = footnoteRid;
                  }
    ...
    summationSections.forEach((section: any) => {
      ...
      paragraphs.forEach((p: any) => {
        ...
        if (match) {
          ...
          // Avoid duplicates
          if (!footnotes.find((fn: any) => fn.originalIdentifier === identifier)) {
            // Get HTML content and clean thoroughly
            let htmlContent = p.innerHTML.trim();
  ...
  linkFootnotes(dom: any, footnotes: any) {
    ...
    footnoteLinks.forEach((link: any) => {
      ...
      const upperRid = footnoteRid.toUpperCase();
      let footnote = footnotes.find((fn: any) =>
      ...
      if (!footnote) {
        ...
        if (numberMatch) {
          ...
          footnote = footnotes.find((fn: any) => fn.originalIdentifier === num);
        }
      }
  ...
  async transformStructure(dom: any, bookId: any) {
    ...
    const tfWrapperSpans = Array.from<any>(dom.querySelectorAll('span.ref-lnk'));
    tfWrapperSpans.forEach((span: any) => {
      unwrap(span);
    });
