...
export class OupProcessor extends BaseFormatProcessor {
  ...
  async extractFootnotes(dom: any, bookId: any) {
    ...
    footnoteElements.forEach((element: any) => {
      ...
      contentClone.querySelectorAll('a[href*="#fn"], .footnote-label, .label').forEach((el: any) => el.remove());
      ...
      contentElement.querySelectorAll('[style]').forEach((el: any) => el.removeAttribute('style'));
  ...
  async extractReferences(dom: any, bookId: any) {
    ...
    if (splitviewItems.length > 0) {
      ...
      splitviewItems.forEach((item: any) => {
        const fullText = item.textContent.trim();

        if (fullText && fullText.length > 10 && !references.find((r: any) => r.content === fullText)) {
          references.push({
            content: fullText,
            originalText: fullText,
            type: 'oup-splitview',
   ...
   * @param {HTMLElement} dom - DOM element
   */
  handleDuplicateTables(dom: any) {
    const modalContainers = dom.querySelectorAll('.table-modal');

    modalContainers.forEach((modalContainer: any) => {
   ...
   * @param {HTMLElement} dom - DOM element
   */
  preserveFigureCaptions(dom: any) {
   ...
   * @param {HTMLElement} dom - DOM element
   */
  removeExtractedSections(dom: any) {
