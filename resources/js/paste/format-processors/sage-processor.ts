...
export class SageProcessor extends BaseFormatProcessor {
  ...
  async extractReferences(dom: any, bookId: any) {
    ...
    if (biblioElements.length > 0) {
      ...
      biblioElements.forEach((element: any) => {
    ...
    if (references.length === 0) {
      ...
      if (citationContainers.length > 0) {
        ...
        citationContainers.forEach((container: any) => {
    ...
    if (references.length === 0) {
      ...
      if (referenceSectionStartIndex !== -1) {
        const elementsToScan = allElements.slice(referenceSectionStartIndex + 1).filter((el: any) => el.tagName === 'P');
    ...
    citationLinks.forEach((link: any) => {
      ...
      if (beforeYear) {
        ...
        if (authorParts.length > 1) {
          const surnames = authorParts.map((part: any) => {
      ...
      if (xmlRid) {
        matchedReference = references.find((ref: any) => ref.xmlRid === xmlRid);
        if (matchedReference) {
