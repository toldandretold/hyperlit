 ...
 * Expects nodeChunks to be an array of chunk objects (generated by your new
 * parser).
 */
 ...
 * Assumes each node has a chunk_id.
 */
function createChunkElement(nodes) {
  ...
  nodes.forEach(node => {
    const html = renderBlockToHtml(node);
...
export function loadPreviousChunkFixed(currentFirstChunkId, instance) {
  ...
  // );
  // the above code looks for an array of chunks, but nodeChunks is just an
  // array of nodes, we must use index/filter instead:
  ...
  //  We must append each individual Node for this to work:
  ...
  // instead we create just one div, and give it a data-chunk-id equal to the
  // nodes chunk ID:
