 ...
 */
export async function extractFileMetadata(file: any) {
...
function titleFromFilename(name: any) {
...
async function extractMarkdown(file: any) {
    const text = await readFileSlice(file, 10 * 1024);
    const result: any = { title: '', author: '', year: '', publisher: '' };
...
function yamlValue(yaml: any, key: any) {
    const re = new RegExp('^' + key + '\\s*:\\s*["\']?(.+?)["\']?\\s*$', 'mi');
    const m = yaml.match(re);
    return m ? m[1].trim() : '';
}

async function extractEpub(file: any) {
    const result: any = { title: '', author: '', year: '', publisher: '' };
...
async function extractEpubFirstHeading(zip: any, opfDoc: any, opfPath: any) {
...
function dcText(doc: any, localName: any) {
...
async function extractDocx(file: any) {
    const result: any = { title: '', author: '', year: '', publisher: '' };
    const JSZip = await loadJSZip();
    const zip = await JSZip.loadAsync(file);
...
async function extractDocxFirstHeading(zip: any) {
...
async function extractHtml(file: any) {
...
function readFileSlice(file: any, maxBytes: any) {
