 ...
 */
function applyThemeClass(themeName: any) {
 ...
 */
export function switchTheme(theme: any) {
  // Validate theme
  ...
  if (theme === THEMES.VIBE) {
    import('./vibeCSS/index').then(m => m.applyVibeCSS());
  } else {
    import('./vibeCSS/index').then(m => m.removeVibeCSS());
  }
