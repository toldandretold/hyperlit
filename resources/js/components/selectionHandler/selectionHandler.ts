...
let hyperlightButtons: any = null;
let originalParent: any = null;
let wordCountDisplay: any = null;
...
function handleSelection() {
  ...
  if (selectedText.length > 0) {
    ...
    if (window.innerWidth <= 768) {
      ...
      if ((window as any).isEditing && editToolbar) {
    ...
    if (!isOverlapping && (window as any).isEditing) {
      const range = selection.getRangeAt(0);
      const container = range.commonAncestorContainer;
      const root: any = container.nodeType === Node.TEXT_NODE ? container.parentElement : container;
