...
class ContainerDragger {
  ...
  startResize(event: any, resizeHandle: any) {
    ...
    if (this.currentContainer.classList.contains('hyperlit-container-stacked')) {
      ...
      if (depth && (window as any).getStackDepthInfo) {
        this.draggedDepthInfo = (window as any).getStackDepthInfo(depth);
      }
    }
  ...
  resize(clientX: any, clientY: any) {
    if (!this.currentContainer) return;
    ...
    if (this.containerType === 'hyperlit-container') {
      ...
      if ((window as any).resizeAllLayers) {
        let newBaseWidth: any;
        ...
        (window as any).resizeAllLayers(newBaseWidth);
  ...
  endDragOrResize() {
    ...
    if (this.currentContainer && this.currentContainer.isConnected && (window as any).containerCustomizer) {
      const viewportWidth = window.innerWidth;
      ...
      if (this.containerType === 'hyperlit-container') {
        ...
        if (base) {
          ...
          (window as any).containerCustomizer.updateContainer('hyperlit-container', customizations);
...
// on re-entry. It stays exposed as (window as any).containerDragger because container lifecycle code
// (containerManager.ts) calls reset() through that global.
export function initContainerDragger() {
  if (!(window as any).containerDragger) {
    (window as any).containerDragger = new ContainerDragger();
  } else {
    (window as any).containerDragger.reset();
  }
  return (window as any).containerDragger;
}
