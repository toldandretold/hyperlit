/**
 * wheelScrollForwarder — make the mouse wheel scroll the home / user page from the
 * "dead zones" outside the centered content column.
 *
 * The only scrollable element on the home and user pages is the content wrapper
 * (`.home-content-wrapper` / `.user-content-wrapper`), which on desktop is a narrow
 * centered column (`max-width: var(--content-width, 40ch); margin: 0 auto`). Two regions
 * therefore have no scroll target under the pointer:
 ...
 * wrapper and preventDefault. Over the reading content itself it does nothing, so native scroll
 ...
 * registerComponents.ts) for pages ['home','user'], NOT a @vite side-effect or a top-level
...
const WRAPPER_SEL = '.home-content-wrapper, .user-content-wrapper';
// scrollable reading content — native scroll already works here, leave it alone
const CONTENT_SEL =
  '.home-content-wrapper .main-content, .user-content-wrapper .main-content, .welcome-copy';
