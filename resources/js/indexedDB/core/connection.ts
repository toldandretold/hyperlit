...
let openingPromise: Promise<IDBDatabase> | null = null; // prevents parallel open races
 ...
 */
export async function openDatabase(retryCount = 0): Promise<IDBDatabase> {
 ...
 * (iOS < 15.4).
 */
export async function withWriteLock<T>(fn: (db: IDBDatabase) => T | Promise<T>): Promise<T | undefined> {
...
async function _openFresh(retryCount = 0): Promise<IDBDatabase> {
      ...
      if (oldVersion < 21) {
        ...
        ALL_STORE_CONFIGS.forEach((storeConfig) => {
          if (!db.objectStoreNames.contains(storeConfig.name)) {
            const storeOptions: IDBObjectStoreParameters = { keyPath: storeConfig.keyPath };
