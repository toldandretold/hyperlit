...
import { showIDBRecoveryToast, updateIDBRecoveryToast, hideIDBRecoveryToast } from './recoveryToast';
...
const failedOperationQueue: Array<() => unknown> = [];
 ...
 */
export function reportIDBSuccess(): void {
 ...
 *
 * @param opts.retryFn  A () => void that re-queues + retries the operation
 * @returns `true` if the caller should **stop** re-queuing (IDB is broken)
 */
export function reportIDBFailure(error: unknown, opts: { retryFn?: () => unknown } = {}): boolean {
  consecutiveFailures++;
...
/**
 * @returns Whether IDB is currently considered broken.
 */
export function isIDBBroken(): boolean {
  return idbBroken;
}
...
async function _doRecovery(): Promise<boolean> {
...
async function _drainFailedOps(): Promise<void> {
