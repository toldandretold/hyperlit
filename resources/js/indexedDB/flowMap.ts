...
export const FLOW_STAGES: FlowStage[] = [
  {
    ...
    modules: [
      ...
      { path: 'nodes/absenceReconciler', plain: 'Save-time hypercite self-heal: diffs a saved node\'s previous content against the live DOM; hypercite <u>/<a> ids that vanished without a mutation event are delinked / tombstoned+ghosted / orphaned (the dropped-mutation-batch bug class).' },
  ...
  },
  {
    ...
    modules: [
      ...
      { path: 'syncQueue/sentSyncTokens', plain: 'localStorage ledger of client-generated sync_token write ids. The server stores the token beside the library timestamp it produces and echoes it in a STALE_DATA 409 — a ledger hit proves the "conflict" is our own committed-but-lost write even after further local edits, where the content compare can\'t.' },
