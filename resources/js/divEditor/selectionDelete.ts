...
export class SelectionDeletionHandler {
  [key: string]: any;
  constructor(editorContainer: any, callbacks: any = {}) {
  ...
  setupListeners() {
    ...
    this.editor.addEventListener('keydown', (e: any) => {
    ...
    this.editor.addEventListener('input', (e: any) => {
   ...
   */
  checkAndUnwrapLinks(e: any) {
    if (!(window as any).isEditing) return false;
    ...
    import('../utilities/operationState').then(({ setProgrammaticUpdateInProgress }) => {
   ...
   */
  async _handleSpecialElementDeletion(range: any, sourceHypercites: any, hyperciteLinks: any, footnotes: any) {
    try {
      ...
      if (hasWarning) {
        ...
        if (footnotes.length > 0) {
          const fnNums = footnotes.map((s: any) => s.getAttribute('fn-count-id')).join(', ');
      ...
      const { setProgrammaticUpdateInProgress } = await import('../utilities/operationState');
      setProgrammaticUpdateInProgress(true);
  ...
  if (!selection.isCollapsed && selection.rangeCount > 0) {
    ...
    console.log('🔍 SELECTION RANGE:', {
      ...
      commonAncestorClass: (range.commonAncestorContainer as any).className,
  ...
  handlePostDeletion() {
    ...
    import('../utilities/operationState').then(module => {
    ...
    if (nodeIdsToDelete.length > 0) {
      ...
      if (this.queueNodeForDeletion) {
        ...
        nodeIdsToDelete.forEach((id: any) => {
    ...
    const additionalNodesToDelete: any[] = [];

    boundaryElementIds.forEach((id: any) => {
...
batchDeleteFromIndexedDB(nodeIds: any) {
  ...
  isEffectivelyEmpty(element: any) {
    return !element.textContent.trim() && 
           !element.querySelector('img, br, hr, video, audio');
  }
  
  findEmptyParagraphs(container: any) {
