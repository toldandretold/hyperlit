...
- **⤓ conversion glitch** — the conversion CODE needs fixing: downloads `<book>.tar.gz`, the complete case (all DB rows incl. annotations + the whole `resources/markdown/<book>/` artifact dir: `original.*`, `ocr_response.json`, `assessment.json` decision trace, debug files, the user complaint).
- **⤓ harvest glitch** — the ACQUISITION needs fixing: the text isn't a mangled version of the work, it isn't the work at all. Use this when the book is a journal landing page, a "prove you're not a robot" interstitial, an abstract with nothing after it, or the wrong edition. See the section below.
