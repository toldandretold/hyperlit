...
- **Rate** — one aggregate charge when the review completes: OCR pages from the pipeline's `step_timings` (priced per served model) + **Brave web-search requests** from `step_timings.web_search.requests` (config `services.brave_search.price_per_1k_requests`, default $5/1k) + **browser page fetches** from `step_timings.browser_fetch.fetches` (config `services.source_fetch.browser_fetch_price`, default $0.01 each) + **unblocked page fetches** from `step_timings.unblocker_fetch.fetches` (config `services.unblocker.price_per_fetch`, default $0.0007 each) + per-model LLM token costs from `llm_usage` (config `services.llm.pricing`, per-1M-token input/output). Itemized in `line_items` (`ocr` / `web_search` / `browser_fetch` / `unblocker_fetch` / `llm`).
- **Where** — `CitationReviewCommand::billReview`, reached via `CitationPipelineJob` → `Artisan::call('citation:pipeline')` with `--user-id`.
- **BYO waiver** — when the pipeline ran in client-inference mode (`citation_pipelines.inference_mode = 'client'`), the LLM lines are WAIVED (the user's own key already paid for the tokens); server-side OCR of sources, **web search and browser fetches** still bill — inference tickets cover the user's LLM provider, but the Brave key and the residential proxy are OURS in every inference mode.
- **Web-search counting** — Brave charges per REQUEST, including ones that resolve nothing (and 402s), so `BraveSearchService` counts on ISSUE rather than inferring from resolutions. `CitationPipelineCommand` resets the counter at the start of a run and writes the total into `step_timings.web_search` BEFORE the review step (the step that bills). `BraveSearchService` is a **container singleton** for the same reason `LlmService` is: the requests happen inside `CitationScanBibliographyJob`'s wave 8 while the charge is assembled later, so a per-resolve instance would read 0 and every web search would be on us. The free $5/month Brave credit is deliberately not modelled — the charge is marginal list price. The citation pipeline is Brave's only consumer (~130 requests ≈ $0.65 for a heavily-cited article).
- **Browser-fetch counting** — when a citation's URL cannot be read by a plain GET (a JS-rendered page, a Cloudflare interstitial), resolution escalates to the headless browser through `ContentFetchService::acquirePageHtml`. That spends a browser process and residential-proxy bandwidth whether or not it recovers an article, so `WebTextAcquirer` counts the ATTEMPT. It is a **container singleton** for the same reason `BraveSearchService` is — the escalations happen inside the bibliography scan's fetch waves while the charge is assembled later in `billReview`. A flat per-fetch rate is deliberate over metered bytes: the proxy's accounting is not visible to us, and a number the user can predict beats a precise one they cannot. Set `BROWSER_FETCH_PRICE=0` to absorb it. Escalation is also time-budgeted for citations (`services.source_fetch.citation_browser_budget_ms`, default 20s) rather than taking harvest's ~62s, because a review runs dozens of URLs and a hard-walled page spending a full minute to conclude "blocked" is a minute of nothing.
- **Unblocker counting** — the managed-unblocker rung (`App\Services\WebContent\UnblockerClient`, dormant until `UNBLOCKER_URL` is set) is the last resort for the ~21-in-59 citation URLs behind managed bot challenges our own patchright stack loses whatever the exit IP. These services bill per SUCCESSFUL retrieval, so unlike the browser counter this one increments on success only — a failure genuinely costs nothing. A 402 from it is logged at error level for the same reason Brave's is: the service is up, the balance is not, and every citation quietly stops resolving.
- **Cited PDFs ride the OCR lane like every other source** — `PdfSourceReader` downloads and STAGES the file (`resources/markdown/{book}/original.pdf`, `pdf_url_status='downloaded'`, `has_nodes=false`), which is exactly what `citation:ocr` selects on, so step 3 converts it with footnotes and headings intact and bills the pages as the normal `ocr` line item. An earlier version read the text layer in-process for free, but produced structureless plain text AND was invisible to the OCR step on all three of its conditions. A scan with no text layer is staged too — OCR is what it is for. Fetching a YouTube transcript (`yt-dlp` metadata plus a caption file) remains free.
