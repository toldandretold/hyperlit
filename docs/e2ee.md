...
## Images (encrypted blobs, not inlined)

Book images live in a unified store: a `book_images` Postgres table (metadata + lifecycle, RLS-governed like `nodes`) plus the bytes on a **private** Flysystem disk (`storage/app/books/{book}/images/`, never public-served — the old EPUB `/storage/books/…` public-symlink path is gone). One RLS-gated route serves them (`BookMediaController`: a `book_images` row invisible under RLS → 404, never leaks existence; public books get `public,max-age`, everything else `private,no-store`; SVG gets a CSP sandbox). One write seam (`BookImageStore::ingestFromDirectory`, called from `ProcessDocumentImportJob` + `ConversionArtifactSaver`) moves every importer's `media/` output in with `prune`. Deletion + reconvert now purge images (they never did). Backfill existing books with `php artisan images:migrate-to-store`.

For an ENCRYPTED book the on-disk bytes are **ciphertext** — data-URI inlining was rejected (bloat + fidelity loss + no lazy-load). The blob envelope is `HLENC1` (6-byte magic) + 12-byte IV + AES-GCM ciphertext, AAD = root book id (`e2ee/crypto.ts encryptBytes/decryptBytes/hasBlobMagic`). The lock pass (`e2ee/imageBlobs.ts`, run from `lockBook` AFTER the content push) downloads each plaintext image, encrypts, and `PUT /api/books/{book}/images/{filename}`s it back (raw octet-stream body); the server's magic guard rejects a plaintext body for an encrypted book and vice-versa (`BookImageController`), then replaces the bytes in place and flips the row's `encrypted` flag. Publish reverses it. Both are idempotent (already-flipped rows skip), so a partial failure just needs a re-run.

Rendering: stored content ALWAYS keeps the canonical `/{book}/media/…` src (DOMPurify strips `blob:` from HTML), and `lazyLoader/encryptedImages.ts hydrateEncryptedImages` — called from `chunkRender` after `handleBrokenImages`, no-op for plaintext books — fetches the ciphertext, decrypts with the book DEK, and assigns `img.src` an object URL via JS property (post-sanitize, never through the HTML string). Blob URLs cache per canonical src for the session (trimWindow churn would thrash per-chunk revocation) and are revoked wholesale by `clearImageBlobCache()` on vault lock (`lockSessionCaches`). The save path can't leak a blob URL: `contentProcessor` restores the canonical src from `data-hl-src` before capturing `content`, so IndexedDB/sync/beacon always store the stable src; the divEditor MutationObserver only watches `style`, so the hydration's `src`/`class` writes don't trigger spurious saves.

Plaintext metadata by design: an encrypted row's `filename`, `mime`, and `width`/`height` stay readable (the route + layout need them). Only the pixel content is protected.
