...
| `container-scroll-stability.spec.js` | Regression for the "reader jumps after closing a hyperlit container" bug. After navigating to a fragment (`#hypercite_X` / `#HL_X`) and opening a container, the close (overlay tap → `history.back()`) let the browser's scroll restoration snap the reader to the URL fragment / a stale pixel. Fix = scoped manual scroll restoration in `hyperlitContainer/core.js` (`history.scrollRestoration = 'manual'` while a container is open, reset to `'auto'` after close). Authors a book with a below-the-fold hyperlight, opens it in read mode, closes via the back path, and asserts BOTH the **mechanism** (`scrollRestoration` is `'manual'` while open, `'auto'` after — deterministic, browser-agnostic) and the **behaviour** (`.reader-content-wrapper` scrollTop unchanged ±3px across open→close). Negative-control verified: failed at the mechanism assertion with the fix disabled. Note: `helpers/stateSnapshot.js` now also records `readerScrollTop` (the real inner scroller — `window.scrollY` is ~always 0 here) and `scrollRestoration` for timeline forensics. |
