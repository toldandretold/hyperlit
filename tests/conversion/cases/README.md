...
A scrape-acquired book (`conversion_method` = `paste_engine_html` / `html_scrape_unverified` / `web_article_*`) was converted by the **shared paste engine** (`scripts/paste-convert.mjs` + `resources/js/paste/format-processors/`), NOT `app/Python` — so if the fetched page is the RIGHT article but the book renders mangled (lost footnotes, broken citations, scrambled structure), neither loop above applies. `book:import-cases` detects this and prints the paste loop; the steps are:

1. The bundle's `artifacts/fetched_page.html` is the ground truth the engine consumed — copy it to `tests/paste/fixtures/clipboard/<publisher>-<slug>.html` (naming convention in that folder's README). Bundles from before fetched-page capture existed have no copy — retract and let the gated ladder re-fetch one.
