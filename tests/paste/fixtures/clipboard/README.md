...
- ScienceDirect (author-date — `sciencedirect.html`): https://www.sciencedirect.com/science/article/pii/S0962629825001209
- ScienceDirect (footnotes — `sciencedirect-footnotes.html`): https://www.sciencedirect.com/science/article/pii/S0305748825000416 — the OTHER SD citation style, 109 notes. Both styles share one anchor vocabulary: a bibliography link is `data-xocs-content-type="reference" data-xocs-content-id="b0120"` and a NOTE marker is the same content-type ided `fn1`, so only the id prefix separates them. Every SD capture before this one was author-date, so `extractFootnotes` returned `[]` and `convertCitationLinks` ate all 109 markers as failed bibliography lookups — notes lost, each marker left as a bare digit welded to its sentence. Contract lives in `tests/paste/format-processors/scienceDirect.footnotes.test.js`.
