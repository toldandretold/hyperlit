 ...
 */
const EXTRA_ROOTS = [path.join(RES_ROOT, 'hyperlights'), path.join(RES_ROOT, 'hypercites'), path.join(RES_ROOT, 'divEditor'), path.join(RES_ROOT, 'editToolbar'), path.join(RES_ROOT, 'footnotes'), path.join(RES_ROOT, 'citations'), path.join(RES_ROOT, 'hyperlitContainer'), path.join(RES_ROOT, 'lazyLoader'), path.join(RES_ROOT, 'scrolling'), path.join(RES_ROOT, 'pageLoad'), path.join(RES_ROOT, 'SPA')];
...
export function collect(): FlowViz {
  ...
  const isAnalyzed = (key: string): boolean =>
    stageOf.has(key) || key.startsWith('hyperlights/') || key.startsWith('hypercites/') || key.startsWith('divEditor/') || key.startsWith('editToolbar/') || key.startsWith('footnotes/') || key.startsWith('citations/') || key.startsWith('hyperlitContainer/') || key.startsWith('lazyLoader/') || key.startsWith('scrolling/') || key.startsWith('pageLoad/') || key.startsWith('SPA/');
  ...
  return {
    ...
    stageOrder: [...FLOW_STAGES.map(s => s.id), 'hyperlights', 'hypercites', 'divEditor', 'editToolbar', 'footnotes', 'citations', 'hyperlitContainer', 'lazyLoader', 'scrolling', 'pageLoad', 'SPA'],
    legend: [
...
function rebuild(){
  ...
  var FOLDER_ORDER=["hyperlights","hypercites","divEditor","editToolbar","indexedDB","footnotes","citations","hyperlitContainer","lazyLoader","scrolling","pageLoad","SPA"];
  var FCOLOR={indexedDB:"#9aa6bd",hyperlights:"#3fb6b6",hypercites:"#caa14b",divEditor:"#5e8fd6",editToolbar:"#d18ad0",footnotes:"#c98a5e",citations:"#7bbf6a",hyperlitContainer:"#c45d6d",lazyLoader:"#6abf9f",scrolling:"#bf8a6a",pageLoad:"#8a7bbf",SPA:"#b06a8a"};
