...
- **`VlmOcrClient.swift`** — local/BYO vision-model mode: each page is rendered and sent to an OpenAI-compatible chat endpoint (Ollama, LM Studio, or hosted) with a transcription prompt; the MODEL decides what is a heading, a footnote, or a page number — the judgment geometric heuristics can't make on messy scans (Apple Vision misreads superscript markers as ®/'). Setup: `brew install ollama && ollama pull qwen2.5vl`, then activate the "Ollama vision (local)" preset in Settings → PDF OCR. Slow (seconds per page) but far better on scanned books; the CLI exposes it as `--vlm <base_url> --vlm-model <model>`.
