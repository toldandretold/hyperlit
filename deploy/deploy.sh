...
on_err() {
    ...
    local before_short="${BEFORE:-unknown}" after_short="${AFTER:-}"
    before_short="${before_short:0:8}"
    if [ -n "${after_short}" ]; then after_short="${after_short:0:8}"; else after_short="(pull did not complete)"; fi
    echo "    code pulled ......... $([ "${BEFORE:-}" = "${AFTER:-}" ] && echo 'no new commits' || echo "${before_short} → ${after_short}")"
...
# The paste-engine bundle is regenerated here by every `npm run build` (so the
# node-side and browser-side engines are the same build), which leaves it
# byte-different from the committed copy and blocks the pull whenever a commit
# touches it. Discard the local copy — the build below recreates it anyway.
if ! git diff --quiet -- scripts/generated/paste-engine.mjs 2>/dev/null; then
    run git checkout -- scripts/generated/paste-engine.mjs
    ok "reset locally-rebuilt scripts/generated/paste-engine.mjs (regenerated by the build step)"
fi
